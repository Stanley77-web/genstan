...
describe('# userController', () => {
    describe('## create method', () => {
        it('Scenario 1', async () => {
            const request = { body: { id: 'uid-13', email: 'test-8@gmail.com', password: 'password-8', name: 'user-8' } }
            const ret = {}
        ...
        it('Scenario 2', async () => {
            const request = { body: { id: 'uid-4', email: 'test-9@gmail.com', password: 'password-9', name: 'user-9' } }
            const ret = {}
            const response = { send: jest.fn(data => ret.send = data), status: jest.fn().mockReturnThis() }
            await userController.create(request, response)
            expect(ret).toEqual({"send":"User not created"})
        ...
        it('Scenario 3', async () => {
            const request = { body: { id: 'uid-10', email: 'test-10@gmail.com', password: 'password-10', name: 'user-10' } }
            const ret = {}
            const response = { send: jest.fn(data => ret.send = data), status: jest.fn().mockReturnThis() }
            await userController.create(request, response)
            expect(ret).toEqual({"send":{"id":"uid-10","email":"test-10@gmail.com","password":"password-10","name":"user-10"}})
        })
    })
    ...
    describe('## findById method', () => {
        ...
        it('Scenario 3', async () => {
            const request = { params: { id: 'uid-11' } }
            const ret = {}
            const response = { send: jest.fn(data => ret.send = data), status: jest.fn().mockReturnThis() }
            await userController.findById(request, response)
            expect(ret).toEqual({"send":{"id":"uid-11","email":"test-6@gmail.com","password":"password-6","name":"user-6"}})
        })
    })
    describe('## findUserAccount method', () => {
        it('Scenario 1', async () => {
            const request = { body: { email: 'test-2@gmail.com', password: 'password-2' } }
            const ret = {}
            const response = { send: jest.fn(data => ret.send = data), status: jest.fn().mockReturnThis() }
            await userController.findUserAccount(request, response)
            expect(ret).toEqual({"send":{"id":"uid-2","email":"test-2@gmail.com","password":"password-2","name":"user-2"}})
        })
        it('Scenario 2', async () => {
            const request = { body: { email: 'test-5@gmail.com', password: 'password-8' } }
            const ret = {}
            const response = { send: jest.fn(data => ret.send = data), status: jest.fn().mockReturnThis() }
            await userController.findUserAccount(request, response)
            expect(ret).toEqual({"send":"User not found"})
        ...
        it('Scenario 3', async () => {
            const request = { body: { email: 'test-3@gmail.com', password: 'password-10' } }
            const ret = {}
            const response = { send: jest.fn(data => ret.send = data), status: jest.fn().mockReturnThis() }
            await userController.findUserAccount(request, response)
            expect(ret).toEqual({"send":"User not found"})
