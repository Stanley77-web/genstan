...
describe('# DistributorController', () => {
    ...
    describe('## updateProduct method', () => {
        it('Scenario 1', async () => {
            const request = { user: {  }, body: { productObj: { productCommercialId: 'ProductCommercialID-010', productType: 'Headphone', quantity: 100, price: 100 } } }
            const ret = {}
            const response = { json: jest.fn(data => ret.json = data), status: jest.fn().mockReturnThis() }
        ...
        it('Scenario 2', async () => {
            const request = { user: {  }, body: { productObj: { productCommercialId: 'ProductCommercialID-001', productType: 'Laptop', quantity: 10, price: 1000 } } }
            const ret = {}
            const response = { json: jest.fn(data => ret.json = data), status: jest.fn().mockReturnThis() }
            await DistributorController.updateProduct(request, response)
            expect(ret).toEqual({"json":{"message":"User not found!","status":"notfound"}})
