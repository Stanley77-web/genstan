...
describe('# ImageController', () => {
    describe('## upload method', () => {
        it('Scenario 1', async () => {
            const request = { body: { imageString: 'ImageString-008', imagePathNameToFirebase: 'ImageNamePath-007' } }
            const ret = {}
        ...
        it('Scenario 2', async () => {
            const request = { body: { imageString: 'ImageString-003', imagePathNameToFirebase: 'ImageNamePath-004' } }
            const ret = {}
            const response = { json: jest.fn(data => ret.json = data), status: jest.fn().mockReturnThis() }
            await ImageController.upload(request, response)
            expect(ret).toEqual({"json":{"message":"successfully","data":null,"error":null}})
    ...
    describe('## generateAndPublishQRCode method', () => {
        it('Scenario 1', async () => {
            const request = { body: { qrCodeEncodeData: 'QRCodeEncodeData-010', storageImageNamePath: 'StorageImageNamePath-010' } }
            const ret = {}
            const response = { json: jest.fn(data => ret.json = data), status: jest.fn().mockReturnThis() }
            await ImageController.generateAndPublishQRCode(request, response)
            expect(ret).toEqual({"json":{"message":"successfully","data":"ImageUrl-010","error":null}})
        })
        it('Scenario 2', async () => {
            const request = { body: { qrCodeEncodeData: 'QRCodeEncodeData-010', storageImageNamePath: 'StorageImageNamePath-004' } }
            const ret = {}
            const response = { json: jest.fn(data => ret.json = data), status: jest.fn().mockReturnThis() }
            await ImageController.generateAndPublishQRCode(request, response)
        ...
        it('Scenario 3', async () => {
            const request = { body: { qrCodeEncodeData: 'QRCodeEncodeData-004', storageImageNamePath: 'StorageImageNamePath-008' } }
            const ret = {}
            const response = { json: jest.fn(data => ret.json = data), status: jest.fn().mockReturnThis() }
            await ImageController.generateAndPublishQRCode(request, response)
            expect(ret).toEqual({"json":{"message":"failed","data":null,"error":null}})
